    ...
    public class SecurityLoginLogic : BaseLogic<SecurityLoginPoco>
    {
        ...
        protected override void Verify(SecurityLoginPoco[] pocos)
        {
            ...
            foreach (var poco in pocos)
            {
                ...
                {
                    exceptions.Add(new ValidationException((int)Code.PasswordLength10orGreater, $"Password for SecurityLogin {poco.Id} cannot be null"));
                }
                else if (poco.Password.Length < 10)
                {
                    exceptions.Add(new ValidationException((int)Code.PasswordLength10orGreater, $"Password for SecurityLogin {poco.Id} must be at least 10 characters."));
                }
                else if (!requiredExtendedPasswordChars.Any(t => poco.Password.Contains(t)))
                {
                    exceptions.Add(new ValidationException((int)Code.PasswordSpecialChar, $"Password for SecurityLogin {poco.Id} must contain an extended character of '$', '*', '#', '_' or '@' ."));
                }

                if (string.IsNullOrEmpty(poco.PhoneNumber))
                {
                    exceptions.Add(new ValidationException((int)Code.PhoneNumberEmpty, $"PhoneNumber for SecurityLogin {poco.Id} is required"));
                }
                else
                {
                    ...
                    {
                        exceptions.Add(new ValidationException((int)Code.PhoneNumberPattern, $"PhoneNumber for SecurityLogin {poco.Id} is not in the required format."));
                    }
                    else
                    {
                ...
                {
                    exceptions.Add(new ValidationException((int)Code.EmailAddressValidation, "EmailAddress for SecurityLogin {poco.Id} is not a valid email address format."));
                }
                ...
                {
                    exceptions.Add(new ValidationException((int)Code.FullNameEmpty, "FullName for SecurityLogin {poco.Id} is required."));
                }
